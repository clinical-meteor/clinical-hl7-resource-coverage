...
JsonRoutes.add("get", "/fhir/Coverage", function (req, res, next) { process.env.DEBUG && console.log('GET /fhir/Coverage', req.query);
  ...
  if (accessToken || process.env.NOAUTH) {
    ...
    process.env.DEBUG && console.log('Coverages.find(id)', Coverages.find(databaseQuery).fetch()); // because we're using BaseModel and a _transform() function
    // Coverages returns an object instead of a pure JSON document // it stores a shadow reference of the original doc, which we're removing here
    var coverageData = Coverages.find(databaseQuery).fetch();
    coverageData.forEach(function(patient){
